...
@section('content')
    ...
    <div class="template">
        <div class="box">
            ...
            <ul>
                @foreach($pics as $p)
                <li><a href="{{url('a/'.$p->Id)}}"  target="_blank"><img src="{{url($p->art_thumb)}}"></a><span>{{$p->art_title}}</span></li>
                @endforeach
            </ul>
        </div>
    </div>
    <article>
        ...
        <div class="bloglist left">
            ...
            <p class="dateview"><span>{{date('Y-m-d H:i:s',$d->art_time)}}</span><span>作者：{{$d->art_editor}}</span></p>
            @endforeach
                <div class="page">
                    {{$data->links()}}
                </div>
        ...
        <aside class="right">
            ...
            <div class="news" style="float: left">
                ...
                <ul class="website">
                    @foreach($links as $l)
                    <li><a href="{{$l->link_src}}" target="_blank">{{$l->link_name}}</a></li>
                    @endforeach
                </ul>
            </div>
